...
pacman::p_load(tidyverse, tidytuesdayR, broom, kableExtra, ggplot2, 
               gganimate, dplyr, tidyr, viridis, ggrepel)
```
...
# Save as a GIF
anim_save("gradient_descent_arrows.gif", animation = animate(plot, fps = 5, width = 800, height = 600))
...
-   **R Implementation:**
    ...
    # Save as a GIF
    anim_save("stochastic_gradient_descent_arrows.gif", animation = animate(plot_sgd, fps = 5, width = 800, height = 600))
...
This visualization starts at the initial point (4,4) and simulates the updates using the momentum optimization technique. At each step:
...
![](rmsprop_optimizer_arrows.gif)

### **Explanation**:

In this visualization:

1.  We start at the initial point (4,4) on the quadratic surface f(x, y) = x\^2 + y\^2.

2.  The RMSProp optimizer updates the learning rate dynamically using a moving average of the squared gradients. This ensures:
...
3.  The momentum-like smoothing term (β) helps prevent the learning rate from decaying too quickly, unlike AdaGrad.
...
1.  The optimizer starts at the initial point (4,4) on the quadratic surface f(x, y) = x\^2 + y\^2.

2.  **Adam** combines the benefits of:
...
3.  Bias correction ensures that the first few updates are unbiased, despite starting with zero moments.
...
library(keras)       # For applying optimizers
library(ggplot2)     # For visualization

# Load the dataset
olympic_data <- read.csv("data/medals_total.csv")

# Remove 'country' and 'country_long' columns
olympic_data <- olympic_data %>% select(-country, -country_long)

# One-hot encoding for 'country_code'
dummies <- dummyVars("~ country_code", data = olympic_data)
encoded_data <- predict(dummies, newdata = olympic_data) %>% as.data.frame()

# Combine encoded data with the rest of the dataset
olympic_data <- bind_cols(encoded_data, olympic_data %>% select(-country_code))

# Define predictors (X) and response (y)
X <- olympic_data %>% select(-Total)
...
input_layer <- layer_input(shape = 95)
...
# Define the input and model architecture
input_layer <- layer_input(shape = 95)
output_layer <- input_layer %>%
  layer_dense(units = 64, activation = 'relu') %>%
  layer_dense(units = 1)
...
# Define optimizers
optimizers <- list(
  "GradientDescent" = optimizer_sgd(),
  "Momentum" = optimizer_sgd(momentum = 0.9),
...
results <- lapply(names(optimizers), function(opt_name) {
  # Recompile the model for each optimizer to reset weights
  model %>% compile(loss = 'mse', optimizer = optimizers[[opt_name]], metrics = 'mae')
  ...
  # Capture training history in a data frame
  data.frame(
...
# Plot with gganimate
p <- ggplot(plot_df, aes(x = Epoch, y = Val_Loss, color = Optimizer)) +
  geom_line(size = 1) +
  labs(title = 'Optimizer Comparison Over Epochs', y = 'Validation Loss', x = 'Epoch') +
  theme_minimal() +
  transition_reveal(Epoch)  # Animate over epochs

# Render the animation
animate(p, nframes = 100, fps = 10, width = 800, height = 600, renderer = gifski_renderer())
